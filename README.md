...
### 송동현 (BE)
> 프로젝트를 기획하고 기능을 구현하면서 많은 문제들이 발생하였습니다. 저희는 이슈가 발생할 때마다 상대방의 의견에 귀기울이고 어떤게 최선인지 같이 고민하면서 프로젝트를 완성했습니다. <br>
> 이번 프로젝트를 통해 GIT이용한 협업방법에 대해 배울 수 있었고, 또한 처음에는 백엔드에서 요청 데이터들을  별도의 전처리없이 전달하도록 만들었습니다. 하지만 그렇게 하면 이미 충분히 많은 프론트엔드 코드가 더욱더 복잡해지고 일관성이 떨어질 수 있으며 네트워크의 대역폭이 증가한다는 사실을 페어와의 소통을 통해 알게 되었습니다.<br>
> 페어와의 소통 덕분에 API를 프론트엔드가 필요한 데이터로 전처리 후 전달하도록 수정하였고 효율적인 데이터 전송하여 프로젝트를 선공적으로 구현할 수 있었습니다.
> 항상 페어의 말에 경청하고 열정적이고 책임감을 가지고 프로젝트를 같이 해준 **지연누나** 감사했습니다!! 짱이였습니다!! <br>
> 이번 프로젝트에서 아쉬었던 점은 view함수를 작성하면서 함수마다 같은 과정을 수행하는 코드를 반복사용한 것을 모듈화하면 더 좋았을거 같은 것과 쿼리 수 좀 더 줄일 수 있지 않았을까 라는 아쉬움이 남았습니다. <br>
